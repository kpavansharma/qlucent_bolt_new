...
import { Search, Star, MapPin, Users, Briefcase, Award, Filter, Calendar, ExternalLink, MessageCircle } from 'lucide-react';
...
const vendors = [
  {
    ...
    description: 'Expert Kubernetes and container orchestration consulting with 5+ years of production experience',
    ...
    teamSize: '15-30',
    founded: 2019,
    specialties: ['Kubernetes', 'Docker', 'Cloud Migration', 'DevOps'],
    services: ['Consulting', 'Training', 'Implementation', 'Support'],
    pricing: '$150-200/hour',
    responseTime: '< 2 hours',
    successRate: 98,
    verified: true,
    featured: true,
    certifications: ['AWS Partner', 'CNCF Member', 'Kubernetes Certified'],
    projects: 89,
    clients: ['Airbnb', 'Spotify', 'Netflix']
  },
  {
    id: 2,
    name: 'AI/ML Experts Inc',
    description: 'Machine learning model development and deployment specialists with focus on production scalability',
    logo: '/api/placeholder/100/100',
    rating: 4.8,
    reviews: 93,
    ...
    teamSize: '10-25',
    founded: 2020,
    specialties: ['TensorFlow', 'PyTorch', 'MLOps', 'Computer Vision'],
    services: ['Model Development', 'Training', 'Deployment', 'Optimization'],
    pricing: '$180-250/hour',
    responseTime: '< 4 hours',
    successRate: 96,
    verified: true,
    featured: true,
    certifications: ['Google Cloud ML', 'AWS ML Specialty', 'NVIDIA Partner'],
    projects: 67,
    clients: ['OpenAI', 'Hugging Face', 'Scale AI']
  },
  {
    id: 3,
    name: 'Security First Consulting',
    description: 'Cybersecurity experts specializing in DevSecOps integration and security automation',
    logo: '/api/placeholder/100/100',
    rating: 4.7,
    reviews: 156,
    location: 'Austin, TX',
    teamSize: '20-50',
    founded: 2018,
    specialties: ['DevSecOps', 'Security Automation', 'Compliance', 'Penetration Testing'],
    services: ['Security Audit', 'Implementation', 'Training', 'Monitoring'],
    pricing: '$120-180/hour',
    ...
    projects: 134,
    clients: ['Bank of America', 'Visa', 'PayPal']
  },
  {
    id: 4,
    name: 'Frontend Masters',
    description: 'Modern frontend development experts with deep React, Vue, and Angular expertise',
    logo: '/api/placeholder/100/100',
    rating: 4.6,
    reviews: 89,
    location: 'Seattle, WA',
    teamSize: '5-15',
    founded: 2021,
    specialties: ['React', 'Vue.js', 'TypeScript', 'Next.js'],
    services: ['Development', 'Code Review', 'Training', 'Architecture'],
    pricing: '$100-150/hour',
    responseTime: '< 1 hour',
    successRate: 97,
    verified: true,
    featured: false,
    certifications: ['React Certified', 'Google Developer Expert', 'Microsoft MVP'],
    projects: 78,
    clients: ['Meta', 'Vercel', 'Netlify']
  },
  {
    id: 5,
    name: 'Data Pipeline Pros',
    description: 'Big data and analytics pipeline specialists with expertise in modern data stack',
    logo: '/api/placeholder/100/100',
    rating: 4.5,
    reviews: 72,
    location: 'Chicago, IL',
    teamSize: '8-20',
    founded: 2020,
    specialties: ['Apache Kafka', 'Spark', 'Data Warehousing', 'ETL'],
    services: ['Pipeline Development', 'Data Modeling', 'Training', 'Optimization'],
    pricing: '$130-170/hour',
    responseTime: '< 6 hours',
    successRate: 95,
    verified: true,
    featured: false,
    certifications: ['Snowflake Partner', 'Databricks Partner', 'Apache Foundation'],
    projects: 45,
    clients: ['Uber', 'Lyft', 'DoorDash']
  },
  {
    id: 6,
    name: 'Monitoring & Observability LLC',
    description: 'Full-stack monitoring and observability implementation with Prometheus, Grafana expertise',
    logo: '/api/placeholder/100/100',
    rating: 4.8,
    reviews: 63,
    location: 'Denver, CO',
    teamSize: '6-15',
    founded: 2019,
    specialties: ['Prometheus', 'Grafana', 'Jaeger', 'OpenTelemetry'],
    services: ['Monitoring Setup', 'Dashboard Creation', 'Training', 'Alerting'],
    pricing: '$110-160/hour',
    responseTime: '< 2 hours',
    successRate: 99,
    verified: true,
    featured: false,
    certifications: ['Prometheus Certified', 'Grafana Certified', 'CNCF Ambassador'],
    projects: 112,
    clients: ['Grafana Labs', 'InfluxData', 'Elastic']
  }
];
...
export default function VendorsPage() {
  ...
  const filteredVendors = vendors.filter(vendor => {
    const matchesSearch = vendor.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         ...
                         vendor.specialties.some(spec => spec.toLowerCase().includes(searchQuery.toLowerCase()));
    
    const matchesSpecialty = selectedSpecialty === 'All' || 
                            vendor.specialties.some(spec => spec.toLowerCase().includes(selectedSpecialty.toLowerCase()));
    const matchesService = selectedService === 'All' || 
                          vendor.services.some(service => service === selectedService);
    const matchesTeamSize = selectedTeamSize === 'All' || vendor.teamSize === selectedTeamSize;
    const matchesVerified = !showVerifiedOnly || vendor.verified;

    return matchesSearch && matchesSpecialty && matchesService && matchesTeamSize && matchesVerified;
  }).sort((a, b) => {
    switch (sortBy) {
      case 'rating':
      ...
      case 'price-low':
        return parseInt(a.pricing.split('-')[0].replace('$', '')) - parseInt(b.pricing.split('-')[0].replace('$', ''));
      case 'price-high':
        return parseInt(b.pricing.split('-')[0].replace('$', '')) - parseInt(a.pricing.split('-')[0].replace('$', ''));
      default:
        return 0;
    }
  });
